...
$(function () {
  ...
  sessionGrid.onDeleting = (session) => smartotekaFabric.KBManager().deleteSession(session)
  ...
  tabsGrid.onDeleting = (tabs) => new Promise(resolve => {
    let session = getSelectedSession()
      ...
      case 'saveSession':
        {
          ...
          let session = {
            date: dateCreation,
            query: $('#add-query').val(),
            tags: selectedTags.map(el => ({ id: el.id, text: el.text })),
            tabs: tabs,
          }
  ...
  $('#close-others-btn').click(() => {
    ...
    getAllTabs()
      .then((tabs) => closeTabs(tabs.filter(tab => session.tabs.findIndex(st => st.id === tab.id) === -1)))
  })

  function gridDragOver(event) {
    let dragSupported = event.dataTransfer.types.length
  ...
  function gridDrop(event, grid) {
    ...
    let jsonData = event.dataTransfer.getData(isIE ? 'text' : 'application/json')
    let tabs = JSON.parse(jsonData)
    ...
    let gridApi = grid == 'session' ? sessionGrid.api : tabsGrid.api
    ...
    let row = gridApi.getRowNode(rowId)
    if (!row) {
    ...
    smartotekaFabric.KBManager().updateSession(row.data)
      .then(_ => gridApi.applyTransaction({
        update: [row.data],
      }))
  }
